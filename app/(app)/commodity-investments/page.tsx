...
function DetailDrawer({
  ...
  const [form, setForm]               = useState({ return_date: '', amount: '', notes: '' });
  const [saving, setSaving]           = useState(false);
  const [updatingStatus, setUpdating] = useState(false);
  const [localInv, setLocalInv]       = useState(inv);

  const profit  = localInv.total_return - localInv.modal_amount;
  ...
  const fetchReturns = async () => {
    setLoadingRet(true);
    const res = await fetch(`/api/commodity-investments/${inv.investment_code}/returns`, { credentials: 'include' });
    const j   = await res.json();
    if (j.success) setReturns(j.data ?? []);
    setLoadingRet(false);
  };

  useEffect(() => { fetchReturns(); }, [inv.investment_code]);

  const addReturn = async () => {
    if (!form.return_date || !form.amount) { toast.error('Tanggal dan jumlah wajib diisi'); return; }
    setSaving(true);
    try {
      ...
      setForm({ return_date: '', amount: '', notes: '' });
      setShowForm(false);
      fetchReturns();
      onUpdated();
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full z-50 flex flex-col bg-white shadow-2xl" style={{ width: 560, borderLeft: '1px solid var(--color-border-soft)' }}>
          ...
          <div>
            ...
            {localInv.project_name && (
              <div className="text-[12px] mt-1" style={{ color: 'var(--color-text-muted)' }}>
                Project: {localInv.project_name}
              </div>
            )}
            <div className="text-[12px]" style={{ color: 'var(--color-text-muted)' }}>
              Tanggal invest: {formatDate(localInv.invest_date)}
            </div>
          </div>
        ...
        {/* Balance strip */}
        <div className="grid grid-cols-3 divide-x px-0" style={{ borderBottom: '1px solid var(--color-border-soft)', background: '#F9F8FF' }}>
          {[
            { label: 'Modal', value: formatRupiah(localInv.modal_amount), color: 'var(--color-text)' },
            { label: 'Total Return', value: formatRupiah(localInv.total_return), color: 'var(--color-text)' },
            {
              label: 'Profit / Loss',
        ...
        <div className="flex-1 overflow-auto">
          <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid var(--color-border-soft)' }}>
            <span className="text-[13px] font-semibold" style={{ color: 'var(--color-text)' }}>
              Riwayat Return ({returns.length})
            </span>
          ...
          {showForm && (
            <div className="mx-4 my-3 p-4 rounded-xl" style={{ background: '#F5F3FF', border: '1px solid #EAE8FF' }}>
              <div className="text-[12px] font-semibold mb-3" style={{ color: 'var(--color-primary)' }}>Catat Return Baru</div>
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  ...
                  <input type="number" className="input" min="0" placeholder="0" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} />
                </div>
              </div>
              <div className="mb-3">
                <label className="input-label">Catatan</label>
                <input type="text" className="input" placeholder="Opsional" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
          ...
          {loadingRet ? (
            ...
            <div className="flex items-center justify-center h-24 text-[13px]" style={{ color: 'var(--color-text-muted)' }}>
              Belum ada return yang dicatat
            </div>
          ) : (
...
export default function CommodityInvestmentsPage() {
  ...
  const TAB_TYPES   = [{ v: '', label: 'Semua' }, { v: 'gold', label: 'Emas' }, { v: 'wood', label: 'Kayu' }];
  const TAB_STATUS  = [{ v: '', label: 'Semua' }, { v: 'active', label: 'Aktif' }, { v: 'completed', label: 'Selesai' }, { v: 'cancelled', label: 'Batal' }];
  ...
  return (
    <div className="space-y-4 max-w-[1100px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
            {summary?.active_count ?? 0} investasi aktif · {summary?.total_count ?? 0} total
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Modal', value: formatRupiah(summary?.total_modal ?? 0), color: 'var(--color-text)', sub: null },
          { label: 'Total Return', value: formatRupiah(summary?.total_return ?? 0), color: '#059669', sub: null },
          {
            label: 'Profit / Loss',
            value: profit === 0 ? 'Impas' : formatRupiah(Math.abs(profit)),
            color: profit > 0 ? '#059669' : profit < 0 ? '#dc2626' : 'var(--color-text)',
            sub: profit > 0 ? 'Untung' : profit < 0 ? 'Rugi' : null,
          },
          ...
          <div key={label} className="card p-4">
            ...
            {sub && <div className="text-[11px] mt-0.5 flex items-center gap-1" style={{ color }}>
              {profit > 0 ? <TrendingUp size={11} /> : <TrendingDown size={11} />} {sub}
            </div>}
          </div>
        ))}
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="tbl-wrapper">
          <table className="tbl">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={9} className="text-center py-8" style={{ color: 'var(--color-text-muted)' }}>Memuat...</td></tr>
              )}
              {!loading && data.length === 0 && (
                <tr><td colSpan={9} className="text-center py-10" style={{ color: 'var(--color-text-muted)' }}>Belum ada investasi</td></tr>
              )}
              {data.map(inv => {
                const pl = inv.total_return - inv.modal_amount;
                return (
                  <tr key={inv.id}>
                    <td>
                      <span className="font-mono text-[11.5px]" style={{ color: 'var(--color-text-secondary)' }}>
                        {inv.investment_code}
                      </span>
                    </td>
                    <td>
                      <span className={`badge ${COMMODITY[inv.commodity_type]?.cls}`}>
                        {COMMODITY[inv.commodity_type]?.label}
                      </span>
                    </td>
                    ...
                    <td style={{ color: 'var(--color-text-muted)', fontSize: 12, whiteSpace: 'nowrap' }}>
                      {formatDate(inv.invest_date)}
                    </td>
                    <td className="text-right tbl-mono">{formatRupiah(inv.modal_amount)}</td>
                    <td className="text-right tbl-mono" style={{ color: '#059669' }}>
                      {formatRupiah(inv.total_return)}
                    </td>
                    <td className="text-right">
                      <span style={{
                        fontFamily: 'monospace', fontSize: 12, fontWeight: 600,
                        color: pl > 0 ? '#059669' : pl < 0 ? '#dc2626' : 'var(--color-text-muted)',
                      }}>
                        {pl === 0 ? '—' : `${pl > 0 ? '+' : '-'}${formatRupiah(Math.abs(pl))}`}
                      </span>
                    </td>
                    <td>
                      <span className={`badge ${STATUS[inv.status]?.cls}`}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[{ v: 'gold', label: 'Emas', cls: '#d97706', bg: '#fffbeb' }, { v: 'wood', label: 'Kayu', cls: '#16a34a', bg: '#f0fdf4' }].map(c => (
                    <button
                      key={c.v}
                      type="button"
                      onClick={() => setForm(f => ({ ...f, commodity_type: c.v }))}
      ...
      {drawerInv && (
        <DetailDrawer
          inv={drawerInv}
          onClose={() => setDrawerInv(null)}
          onUpdated={() => { refetch(); fetchSummary(); }}
        />
      )}
    </div>
  );
