...
interface Supplier { id: number; supplier_code: string; supplier_name: string; phone: string; email: string; address: string; contact_person: string; bank_name: string; account_number: string; created_at: string; }
...
export default function SuppliersPage() {
  ...
  const [form, setForm] = useState({ name:'', phone:'', email:'', address:'', contact_person:'', bank_name:'', account_number:'' });
  ...
  return (
    <div className="space-y-4 max-w-[1100px]">
      ...
      <div className="card overflow-hidden">
        <div className="tbl-wrapper"><table className="tbl"><thead><tr><th>Kode</th><th>Nama</th><th>Contact Person</th><th>Telepon</th><th>Email</th><th>Alamat</th><th>Bank</th><th>Dibuat</th><th></th></tr></thead>
          <tbody>
            ...
            {data.map((s)=>(
              <tr key={s.id}>
                ...
                <td><div className="font-medium" style={{color:'var(--color-text)'}}>{s.supplier_name}</div></td>
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex justify-end" style={{background:'rgba(0,0,0,0.3)'}} onClick={()=>setDetail(null)}>
          <div className="bg-white h-full w-[400px] shadow-xl p-5 overflow-y-auto" onClick={(e)=>e.stopPropagation()}>
            ...
            {[['Kode',detail.supplier_code],['Nama',detail.supplier_name],['Contact',detail.contact_person||'-'],['Telepon',detail.phone||'-'],['Email',detail.email||'-'],['Alamat',detail.address||'-'],['Bank',detail.bank_name||'-'],['No. Rek',detail.account_number||'-'],['Dibuat',formatDate(detail.created_at)]].map(([k,v])=>(
