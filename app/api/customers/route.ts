...
export const GET = withAuth(async (req: NextRequest) => {
  try {
    ...
    if (search) { conditions.push('(name LIKE ? OR email LIKE ? OR customer_code LIKE ?)'); params.push(`%${search}%`,`%${search}%`,`%${search}%`); }

    const where = conditions.join(' AND ');
    const [rows, count] = await Promise.all([
      query(`SELECT * FROM customers WHERE ${where} ORDER BY name ASC LIMIT ? OFFSET ?`, [...params, limit, offset]),
...
export const POST = withAuth(async (req: NextRequest) => {
  try {
    ...
    await query(`INSERT INTO customers (customer_code,name,phone,email,address,city,type,contact_person,tax_id,is_deleted,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,0,NOW(),NOW())`,
