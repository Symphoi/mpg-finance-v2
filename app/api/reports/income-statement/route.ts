...
export const GET = withAuth(async (req: NextRequest) => {
  try {
    ...
    const coa: any[] = await query(
      `SELECT account_code, account_name, account_type FROM chart_of_accounts WHERE account_type IN ('revenue','expense') AND is_active=1 ORDER BY account_code`,
      []
    );
    ...
    const rows = coa.map((acc: any) => {
      ...
      const balance = acc.account_type === 'revenue' ? cr - d : d - cr;
      return { ...acc, total_debit: d, total_credit: cr, balance };
    }).filter((r: any) => r.balance !== 0 || r.total_debit !== 0 || r.total_credit !== 0);

    const revenues = rows.filter((r: any) => r.account_type === 'revenue');
    const expenses = rows.filter((r: any) => r.account_type === 'expense');
    const total_revenue = revenues.reduce((s: number, r: any) => s + r.balance, 0);
    const total_expense = expenses.reduce((s: number, r: any) => s + r.balance, 0);
    const net_income    = total_revenue - total_expense;

    return ok({ from_date: dateFrom, to_date: dateTo, revenues, expenses, total_revenue, total_expense, net_income });
