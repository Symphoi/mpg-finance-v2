...
export default function LoginForm({ site }: { site: SiteSettings }) {
  ...
  const firstLetter = (site.app_name?.[0] ?? 'M').toUpperCase();
  const bgStyle     = site.login_bg_url
    ? { backgroundImage: `url(${site.login_bg_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : { background: 'linear-gradient(135deg,#1a0a3c 0%,#2d1060 60%,#1e1358 100%)' };
    ...
    <div className="min-h-screen flex items-center justify-center" style={bgStyle}>
      <div className="w-full max-w-[400px] p-6">
        ...
        <div className="flex items-center gap-3 mb-8 justify-center">
          <div className="w-10 h-10 rounded-[12px] flex items-center justify-center text-white font-bold text-base overflow-hidden flex-shrink-0"
            style={{ background: site.logo_url ? 'transparent' : 'linear-gradient(135deg,#a855f7,#6366f1)' }}>
            {site.logo_url
          ...
          <div>
            <div className="text-white font-bold text-[18px]">{site.app_name || 'Finance'}</div>
            <div className="text-white/40 text-[11px]">{site.app_subtitle || 'v2.0 Management System'}</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="email" className="input" placeholder="email@mpg.com"
        ...
        <p className="text-center text-white/30 text-[11px] mt-5">
          {site.app_name || 'Finance'} © {new Date().getFullYear()}
        </p>
      </div>
    </div>
  );
